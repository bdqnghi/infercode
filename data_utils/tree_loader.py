...
class TreeLoader(BaseTreeUtils):
    ...
    def load_program_data(self, code_data_directory, subtree_vocab_directory):
        
        for subdir , dirs, files in os.walk(tree_directory): 
            for file in tqdm(files):
                
                code_snippet_path = os.path.join(subdir,file)
                ast_representation = self.load_tree_from_pickle_file(pkl_file_path)
    ...
    def transform_tree_to_index(self, tree_data):
        ...
        data["node_tokens_text"] = node_tokens_text
        data["children_indices"] = children_indices
        data["children_node_types"] = children_node_types
    ...
    def make_batch(self, batch_data):
        ...
        for tree_data in batch_data:
            tree_data = self.transform_tree_to_index(tree_data)
            
            # random_subtree_id = self.random_sampling_subtree(tree_data["subtrees_ids"])
            batch_subtree_id.append(tree_data["subtree_id"])

            batch_node_indexes.append(tree_data["node_indexes"])
            batch_node_types.append(tree_data["node_types"])
            batch_node_tokens.append(tree_data["node_tokens"])
            batch_node_tokens_text.append(tree_data["node_tokens_text"])
            batch_children_indices.append(tree_data["children_indices"])
            batch_children_node_types.append(tree_data["children_node_types"])
            batch_children_node_tokens.append(tree_data["children_node_tokens"])
            batch_tree_size.append(tree_data["size"])
            batch_file_path.append(tree_data["file_path"])
            batch_token_ids.append(tree_data["token_ids"])
    ...
    def process_snippets_to_tensors(self, code_snippets):
